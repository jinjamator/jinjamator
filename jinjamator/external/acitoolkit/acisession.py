...
class Session(object):
    ...
    def __init__(
        ...
        if key and cert_name:
            if NO_OPENSSL:
                raise ImportError(
                    'Cannot use certificate authentication because pyopenssl is not available.\n\
            ...
            try:
                ...
                \nAre you sure you provided the private key? (Not the certificate)"
                    % self.key
                )
        else:
    ...
    def login(self, timeout=None):
        ...
        try:
            ...
            resp.status_code = 404
            resp._content = (
                '{"error": "Could not relogin to APIC due to ConnectionError"}'
            )
    ...
    def get(self, url, timeout=None):
        ...
        if resp.status_code == 403:
            ...
            and "Unable to process the query, result dataset is too big" in resp.text
        ):
