    ...
    if hasattr(obj, "hostFolder"):
        if "ComputeResource" in obj.hostFolder.childType:
            if "cluster" in kwargs and kwargs["cluster"]:
                ret.update(
                    vmware.vsphere.recurse_child_dict(
                        obj.hostFolder, "vim.ClusterComputeResource", key
                    )
                )
            if "standalone" in kwargs and kwargs["standalone"]:
                ret.update(
                    vmware.vsphere.recurse_child_dict(
                        obj.hostFolder, "vim.ComputeResource", key
                    )
                )
            return ret
            ...
            log.info(
                f"Could not find expected childType 'ComputeResource' in {obj.hostFolder.childType}"
            )
            return dict()
    else:
        return dict()
...
def get_list(obj, **kwargs):
    ...
    if vc:
        if "cluster" in kwargs and kwargs["cluster"]:
            ret = ret + vmware.vsphere.view.container(
                vc, obj, vim.ClusterComputeResource, recurse=True
            )
        if "standalone" in kwargs and kwargs["standalone"]:
            ret = ret + vmware.vsphere.view.container(
                vc, obj, vim.ComputeResource, recurse=True
            )
        return ret
    else:
        if hasattr(obj, "hostFolder"):
            if "ComputeResource" in obj.hostFolder.childType:
                if "cluster" in kwargs and kwargs["cluster"]:
                    ret.append(
                        vmware.vsphere.recurse_child_list(
                            obj.hostFolder, "vim.ClusterComputeResource"
                        )
                    )
                if "standalone" in kwargs and kwargs["standalone"]:
                    ret.append(
                        vmware.vsphere.recurse_child_list(
                            obj.hostFolder, "vim.ComputeResource"
                        )
                    )
                return ret
...
def get(attr_name, attr_value, obj, **kwargs):
    ...
    log.debug(
        f"Could not find object matching the given attributes {attr_name} : {attr_value}"
    )
    return False
