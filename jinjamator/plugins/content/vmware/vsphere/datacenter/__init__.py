...
def get_dict(obj, **kwargs):
    ...
    if hasattr(obj, "rootFolder"):
        ...
        log.debug(
            f"We on the vcenter. Going into the rootFolder, prepending {key_prepend}"
        )
        ...
        return vmware.vsphere.recurse_child_dict(
            obj, "vim.Datacenter", key, key_prepend=key_prepend
        )
...
def get_list(obj, **kwargs):
    ...
    if hasattr(obj, "rootFolder"):
        ...
        if vc:
            ret = vmware.vsphere.view.container(vc, obj, vim.Datacenter, recurse=True)
        else:
            ret = []
